...
//#########################################################################################################
// data需调用方准备
bool Wifi_TcpIp_GetDataTcp(uint8_t LinkId, uint16_t *dataLen, uint8_t *data, uint32_t timeout)
{
	osSemaphoreWait(WifiSemHandle, osWaitForever);
	uint8_t result;
	bool returnVal = false;
	...
	while (1)
	{
		if (Wifi.GotNewData)
		{
			if ((strstr(Wifi.TcpIpConnections[LinkId].Type, "TCP") != NULL) && (Wifi.RxDataConnectionNumber == LinkId))
			{
				*dataLen = Wifi.RxDataLen;
				memcpy(data, Wifi.RxBufferForData, Wifi.RxDataLen);
				break;
			}
		}
		if (HAL_GetTick() - begintime >= timeout)
			goto exit;
	}
	returnVal = true;
exit:
...
//#########################################################################################################
// data需调用方准备
bool Wifi_TcpIp_GetDataUdp(uint8_t LinkId, uint16_t *dataLen, uint8_t *data, uint32_t timeout)
{
	osSemaphoreWait(WifiSemHandle, osWaitForever);
	uint8_t result;
	bool returnVal = false;
	...
	while (1)
	{
		if (Wifi.GotNewData)
		{
			if ((strstr(Wifi.TcpIpConnections[LinkId].Type, "UDP") != NULL) && (Wifi.RxDataConnectionNumber == LinkId))
			{
				*dataLen = Wifi.RxDataLen;
				memcpy(data, Wifi.RxBufferForData, Wifi.RxDataLen);
				break;
